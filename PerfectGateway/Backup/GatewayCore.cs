  ...
  public class GatewayCore : GatewayCoreBase
  {
    ...
    public override void CheckAccount(ref Context context)
    {
      ...
      try
      {
        ...
        {
          Logger.Instance.WriteMessage("\r\n\r\nPerfect get client details end", 1);
          ...
          Logger.Instance.WriteMessage("\r\n\r\nPerfect check account numeroCompte" + numerocompte, 1);
          ...
          Logger.Instance.WriteMessage("\r\n\r\nPerfect check account requestMethod " + (requestMethod.ToString() + " " + string.Format("{0}", context[(object) "Method"])), 1);
          ...
          Logger.Instance.WriteMessage("\r\n\r\nMontant de la transaction: " + str1, 1);
          Logger.Instance.WriteMessage("\r\n\r\nPayment value : " + (num.ToString() + " " + str1), 1);
          string str2 = Guid.NewGuid().ToString();
          Logger.Instance.WriteMessage("\r\n\r\nStart Mimoye finance check Account " + (requestMethod.ToString() + " " + pinotp), 1);
          switch (requestMethod)
          {
            case RequestMethod.ConsultationSolde:
              if (pinotp == "0" || pinotp == "")
              {
                Logger.Instance.WriteMessage("\r\n\r\nPerfect check account, start generation ", 1);
            ...
            case RequestMethod.Retrait:
              ...
              if (billCountToDispense.CanDispense)
              {
                ...
                ResponseModel<RetraitCompteClientResponse> responseModel3 = Service.RetraitCompeClient(str2, numerocompte, str1, 11, fraisOperation, pinotp, "Retrait");
                ...
                BillHelper.Transaction(Id, CodeClient2, NumeroCompte, " ", 0, 0, 0, 0, int32, kioskId, OpeRef, status2, 8, description);
                break;
              }
              context.Status = IBP.SDKGatewayLibrary.State.AccountNotExists;
              ...
              context.Description = "Solde guichet insuffisant";
              Logger.Instance.WriteMessage("\r\n\r\nWithdraws description  " + context.Description, 1);
              context[(object) "erreur"] = (object) context.Description;
              ...
              BillHelper.Transaction(Id1, CodeClient3, NumeroCompte1, " ", 0, 0, 0, 0, int32_1, kioskId1, OpeRef1, status4, 8, description1);
              break;
            case RequestMethod.TransfertEntreCompte:
              if (pinotp == "0" || pinotp == "")
              {
                ...
                Logger.Instance.WriteMessage("\r\n\r\nGeneration done2 entre compte ", 1);
                context.Status = IBP.SDKGatewayLibrary.State.AccountExists;
                break;
              }
              Logger.Instance.WriteMessage("\r\n\r\n virement  test", 1);
              string str3 = context[(object) "numeroCompteCredit"].ToString();
              Logger.Instance.WriteMessage("\r\n\r\nPin 3 " + pinotp, 1);
              Logger.Instance.WriteMessage("\r\n\r\nData virement  " + (str2 + " " + numerocompte + str3 + str1 + pinotp), 1);
              ResponseModel<TransfertEntreCompteResponse> responseModel4 = Service.TransfertEntreCompte(str2, numerocompte, str3, str1, 14, "0", pinotp, "Transfert entre comptes ");
              if (responseModel4.Data.CodeReponse == "100000")
              {
                ...
                context[(object) "erreur"] = (object) context.Description;
                BillHelper.Transaction(Guid.NewGuid(), CodeClient1, numerocompte, str3, 0, 0, 0, 0, Convert.ToInt32(num), new Guid(string.Format("{0}", context[(object) "PaymentContext.Payment.Point.Id"])), str2, context.Status.ToString(), 14, context.Description);
                break;
              }
              context.Status = IBP.SDKGatewayLibrary.State.AccountNotExists;
              context.Description = responseModel4.Data.DetailReponse;
              ...
              context[(object) "erreur"] = (object) context.Description;
              BillHelper.Transaction(Guid.NewGuid(), CodeClient1, numerocompte, " ", 0, 0, 0, 0, Convert.ToInt32(num), new Guid(string.Format("{0}", context[(object) "PaymentContext.Payment.Point.Id"])), str2, context.Status.ToString(), 14, context.Description);
              break;
            default:
      ...
      try
      {
        ...
        switch (codeOperation)
        {
          ...
          case CodeOperation.Depot:
            ResponseModel<DepotCompteClientResponse> responseModel1 = Service.DepotCompteClient(str1, numerocompte, montant, 10, fraisOperation, "Depot");
            ...
            BillHelper.Transaction(Guid.NewGuid(), CodeClient, numerocompte, " ", 0, 0, 0, 0, Convert.ToInt32(num), new Guid(string.Format("{0}", context[(object) "PaymentContext.Payment.Point.Id"])), str1, context.Status.ToString(), 3, context.Description);
            break;
    ...
    private void GetClientDetails(ref Context context)
    {
      ...
      try
      {
        ...
        string data = JsonConvert.SerializeObject((object) new
        {
          ...
          apikey = Convert.ToBase64String(Encoding.UTF8.GetBytes(BaseContext.CodeAgent + ":" + BaseContext.PassAgent)),
